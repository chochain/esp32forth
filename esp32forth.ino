...
ForthVM *vm =  new ForthVM(forth_in, forth_out);
...
void loop(void) {
    ...
    if (Serial.available()) {
        ...
        vm->outer();                 // invoke FVM outer interpreter

        Serial.print(forth_out.str().c_str());  // fetch FVM outout stream
        forth_out.str("");           // reset output stream
    }
}
